...
export default function PostsPage() {
  ...
  <select value={selectedLocation} onChange={handleLocationChange} style={{ textAlign:"center", border:"6px solid transparent", outline: "2px solid black", marginRight:"20px" , borderRadius:"12px", backgroundColor:"#AD343E" }}>
    <option value="">All Locations</option>
    {Array.from(new Set(dishes.map((dish) => dish.restaurantLocation))).map((location) => (
      <option key={location} value={location}>
        {location}
  ...
  {/* Cuisine type dropdown */}
  <select value={selectedCuisine} onChange={handleCuisineChange} style={{ textAlign:"center", border:"6px solid transparent", outline: "2px solid black" , borderRadius:"12px" , backgroundColor:"#AD343E"}}>
    <option value="">All Cuisine Types</option>
    {Array.from(new Set(dishes.map((dish) => dish.cuisineType))).map((cuisine) => (
      <option key={cuisine} value={cuisine}>
        {cuisine}
